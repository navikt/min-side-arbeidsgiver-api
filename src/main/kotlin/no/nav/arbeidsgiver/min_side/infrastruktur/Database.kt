...
open class Database(val config: DatabaseConfig) : AutoCloseable {
    val dataSource = HikariDataSource(config.asHikariConfig())
...
class ParameterSetters(
    ...
    fun nullableDate(value: LocalDate?) =
        preparedStatement.setDate(index++, value?.let { Date.valueOf(it) })
